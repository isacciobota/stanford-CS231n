...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c_out = dnext_c + o * (1 - np.power(np.tanh(next_c), 2)) * dnext_h
    dout = np.concatenate((dnext_c_out * g * i * (1 - i),
                           dnext_c_out * prev_c * f * (1 - f),
                           dnext_h * np.tanh(next_c) * o * (1 - o),
                           dnext_c_out * i * (1 - np.power(g,2))), axis=1)
    
    dx = np.dot(dout, Wx.T)
    dprev_h = np.dot(dout, Wh.T)
    dprev_c = (dnext_c_out) * f
    dWx = np.dot(x.T, dout)
    dWh = np.dot(prev_h.T, dout)
