...
def discriminator_loss(logits_real, logits_fake):
    ...
    labels_true = torch.ones(N).to('cuda')
    loss_real = bce_loss(logits_real, labels_true)
    
    labels_fake = torch.zeros(N).to('cuda')
...
def generator_loss(logits_fake):
    ...
    N = logits_fake.shape
    labels_fake = torch.ones(N).to('cuda')
...
def run_a_gan(D, G, D_solver, G_solver, discriminator_loss, generator_loss, loader_train, show_every=250, 
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            D_solver.zero_grad()
            real_data = x.to('cuda')
            logits_real = D(2* (real_data - 0.5)).to('cuda')

            g_fake_seed = sample_noise(batch_size, noise_size).to('cuda')
