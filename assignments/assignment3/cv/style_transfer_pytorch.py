...
def gram_matrix(features, normalize=True):
    ...
    features_flattent = features.view(N, C, -1)
    gram_matrix = torch.bmm(features_flattent, features_flattent.transpose(1, 2))
    # must use toch.bmm instead of np.dot because numpy array is not compatible 
    # with torch.cuda.FloatTensor 
    if normalize:
        gram_matrix /= (H * W * C)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        ...
        style_loss += style_weights[i] * torch.sum((g_matrix - style_targets[i])**2)
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    height_variation_loss = torch.sum((img[:,:,:,:-1] - img[:,:,:,1:]) ** 2)
    width_variation_loss = torch.sum((img[:,:,:-1,:] - img[:,:,1:,:]) ** 2)
    loss = tv_weight * (height_variation_loss + width_variation_loss)
